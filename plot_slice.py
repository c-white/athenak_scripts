...
def main(**kwargs):
  ...
  with open(kwargs['data_file'], 'rb') as f:
    ...
    if kwargs['variable'][:8] == 'derived:':
      ...
      if variable_name not in derived_dependencies:
        raise RuntimeError('Derived variable "{0}" not valid; options are {{{1}}}.'\
            .format(variable_name, ', '.join(derived_dependencies.keys())))
      variable_names = []
      variable_inds = []
    ...
    while f.tell() < file_size:
      ...
      if first_time:
        ...
        slice_normalized_coord = \
            (kwargs['location'] - slice_location_min) / (slice_location_max - slice_location_min)
        first_time = False
  ...
  for block_num in range(num_blocks_used):
    plt.imshow(quantity[block_num], cmap=kwargs['cmap'], norm=norm, vmin=vmin, vmax=vmax, \
  ...
  if kwargs['horizon'] or kwargs['horizon_mask']:
    ...
    if kwargs['dimension'] in ('x', 'y') and kwargs['location'] ** 2 < r_hor ** 2 + bh_a ** 2:
      ...
      if kwargs['horizon']:
        horizon = patches.Ellipse((0.0, 0.0), full_width, full_height, \
            linestyle=horizon_line_style, facecolor='none', edgecolor=kwargs['horizon_color'])
        plt.gca().add_artist(horizon)
    if kwargs['dimension'] == 'z' and abs(kwargs['location']) < r_hor:
      radius = ((r_hor ** 2 + bh_a ** 2) * (1.0 - kwargs['location'] ** 2 / r_hor ** 2)) ** 0.5
      if kwargs['horizon_mask']:
        horizon_mask = patches.Circle((0.0, 0.0), radius=radius, \
            facecolor=kwargs['horizon_mask_color'], edgecolor='none')
        plt.gca().add_artist(horizon_mask)
      if kwargs['horizon']:
        horizon = patches.Circle((0.0, 0.0), radius=radius, linestyle=horizon_line_style, \
  ...
  if kwargs['ergosphere']:
    ...
    if kwargs['dimension'] in ('x', 'y') and kwargs['location'] ** 2 < 4.0 + bh_a ** 2:
      w = np.linspace(abs(kwargs['location']), (4.0 + bh_a ** 2) ** 0.5, ergosphere_num_points)
      z = np.empty_like(w)
      for ind, w_val in enumerate(w):
        def residual_hor(z_val):
          rr2 = w_val ** 2 + z_val ** 2
          r2 = 0.5 \
              * (rr2 - bh_a ** 2 + ((rr2 - bh_a ** 2) ** 2 + 4.0 * bh_a ** 2 * z_val ** 2) ** 0.5)
          return r2 - r_hor ** 2
      ...
      plt.plot(xy_plot, z_plot, linestyle=ergosphere_line_style, linewidth=ergosphere_line_width, \
          color=kwargs['ergosphere_color'], zorder=0)
    if kwargs['dimension'] == 'z' and abs(kwargs['location']) < r_hor:
      def residual_ergo(r):
        return r ** 4 - 2.0 * r ** 3 + bh_a ** 2 * kwargs['location'] ** 2
      r_ergo = brentq(residual_ergo, r_hor, 2.0)
      radius = ((r_ergo ** 2 + bh_a ** 2) * (1.0 - kwargs['location'] ** 2 / r_ergo ** 2)) ** 0.5
      ergosphere = patches.Circle((0.0, 0.0), radius=radius, linestyle=ergosphere_line_style, \
          linewidth=ergosphere_line_width, facecolor='none', edgecolor=kwargs['ergosphere_color'])
...
if __name__ == '__main__':
  ...
  parser.add_argument('variable', help='name of variable to be plotted, any valid derived quantity \
      prefaced by "derived:"')
  ...
  parser.add_argument('--x2_max', type=float, help='vertical coordinate of top edge of plot')
